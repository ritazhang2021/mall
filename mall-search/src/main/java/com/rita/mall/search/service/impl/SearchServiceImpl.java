...
import com.rita.mall.search.feign.ProductFeignService;
import com.rita.mall.search.service.SearchService;
import com.rita.mall.search.vo.AttrResponseVo;
    ...
    private SearchResult bulidSearchResult(SearchParam searchParam, SearchResponse searchResponse) {
        ...
        if (attrs != null && attrs.size() > 0) {
            List<SearchResult.NavVo> navVos = attrs.stream().map(attr -> {
                ...
                navVo.setLink("http://search.mall.com/search.html" + (replace.isEmpty()?"":"?"+replace));
                return navVo;
